...
export class InitialSchema1783364951179 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`products\` (\`id\` bigint UNSIGNED NOT NULL AUTO_INCREMENT, \`uuid\` varchar(36) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`slug\` varchar(191) NOT NULL, \`sku\` varchar(64) NOT NULL, \`name\` varchar(191) NOT NULL, \`price\` decimal(12,2) NOT NULL, \`price_wholesale\` decimal(12,2) NULL, \`stock\` int UNSIGNED NOT NULL DEFAULT '0', \`view_count\` int UNSIGNED NOT NULL DEFAULT '0', \`badge\` enum ('BARU', 'HOT', 'TERLARIS', 'PRE_ORDER') NULL, \`description\` text NOT NULL, \`compatibility\` json NOT NULL, \`specs\` json NOT NULL, \`is_featured\` tinyint NOT NULL DEFAULT 0, \`is_published\` tinyint NOT NULL DEFAULT 1, \`category_id\` bigint UNSIGNED NOT NULL, \`author_id\` bigint UNSIGNED NULL, UNIQUE INDEX \`IDX_uuid\` (\`uuid\`), UNIQUE INDEX \`IDX_products_slug\` (\`slug\`), UNIQUE INDEX \`IDX_products_sku\` (\`sku\`), INDEX \`IDX_products_category\` (\`category_id\`), INDEX \`IDX_products_author\` (\`author_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
